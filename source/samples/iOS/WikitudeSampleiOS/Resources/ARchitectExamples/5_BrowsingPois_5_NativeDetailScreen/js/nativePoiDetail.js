...
var World = {
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
		...
		World.markerDrawable_idle = new AR.ImageResource("assets/icon_bus_idle.png");
		World.markerDrawable_selected = new AR.ImageResource("assets/icon_bus_selected.png");
		World.markerDrawable_directionIndicator = new AR.ImageResource("assets/indi.png");

		// loop through POI-information and create an AR.GeoObject (=Marker) per POI
		for (var currentPlaceNr = 0; currentPlaceNr < staticPOI.length; currentPlaceNr++) {
			var markerPOI = {
				"id": poiData[currentPlaceNr].id,
				"latitude": parseFloat(staticPOI[currentPlaceNr].latitude),
				"longitude": parseFloat(staticPOI[currentPlaceNr].longitude),
				"altitude": parseFloat(staticPOI[currentPlaceNr].altitude),
				"title": staticPOI[currentPlaceNr].name,
				"description": staticPOI[currentPlaceNr].description
			};

			World.markerList.push(new Marker(markerPOI));
		}
